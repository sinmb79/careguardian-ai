...
4. The non-medical release checker inventories tracked and non-ignored untracked release files. Whole-file exceptions were replaced by exact single-line policy/legacy/cloud-disable/network contracts plus TypeScript AST constant folding for split names, templates, concatenation, array `join`, aliases, computed global access, and fail-closed dynamic calls. `Reflect`, `eval`, `Function`, `Proxy`, computed invocation, global-object capability acquisition, hidden bare network capabilities, and direct or aliased built-in facade mutation are rejected. Every `expo-file-system` import, binding, member, and call is denied except the exact `expo-file-system/legacy` usages already present in the three approved files; the resumable download task requires its complete argument/callback/return graph. Upload APIs, the new `File.downloadFileAsync` surface, aliases, destructuring, computed members, and extra imports are rejected. Installed version `19.0.23` and the public runtime declarations reachable from both main and legacy entry points are pinned, so a dependency upgrade or newly exposed runtime value fails closed. Global `require` is accepted only as the direct callee of the nine exact static-string calls already present; loader assignment, aliasing, passing, return, destructuring, sequence, `call`/`apply`/`bind`, computed/member access, `createRequire`, `node:module`, and non-contract direct calls are rejected. The two required CommonJS `module.exports` statements are separately fixed by exact contracts.
...
| `npm run verify` | PASS — 21 Vitest files / 189 tests; build; all generated HTML CSP; mobile typecheck; Expo doctor 18/18; policy/model; 46 mutation/contract tests; exact audit; YAML workflow contract |
| `npm run release:gate-tests` | PASS — 46 Node mutation/contract tests (3 static + 18 policy + 10 model + 6 audit + 9 workflow) |
| `npm run release:audit-policy` | PASS — exact accepted baseline: Critical 0, High 19, Moderate 10, 29 packages, 6 GHSA IDs |
| Independent YAML parse | PASS — root/job/verify exact key sets and `deploy.needs: verify` confirmed |
