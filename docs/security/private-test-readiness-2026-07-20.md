...
Play에 올리기 전 남은 외부 게이트:

1. 개인정보처리방침 URL의 HTTP 200과 앱 내 링크를 확인한다.
2. Play의 Health를 `Medication and Treatment Management`, 카테고리를 `Medical`, 광고 ID를 미사용으로 정정한다.
3. Data safety의 “수집 없음/공유 없음”은 최종 AAB와 현재 단일 에뮬레이터 관찰에는 일치한다. 비공개 테스트 중에도 실기기 네트워크 관찰을 계속한다.
4. 보스의 확인 후에만 AAB 업로드·저장·검토 제출을 실행한다.
...
- [ ] 개인정보처리방침 URL 200 및 인앱 링크
- [ ] 모바일/웹 실제 데이터 흐름과 정책 일치
- [ ] Play Health 선언 최소 Medication and Treatment Management
- [ ] Closed testing이면 Data safety 제출
- [ ] 의료기기가 아니며 진단·치료·처방 검증을 하지 않는다는 disclaimer
- [ ] 실제 구현만 말하는 Store/UI/테스터 설명
...
- [ ] dev-client 중복·production 포함 제거
- [ ] Expo 호환 patch 및 의존성 advisory 처리
- [ ] 사용하지 않는 권한·exported component 제거
- [ ] 새 AAB manifest/dex diff
- [ ] 실제 기기 네트워크 capture에서 선언과 동작 일치
- [ ] AAB hash와 SBOM/감사 결과 보관
...
- 최초 감사에서는 Play Console을 열지 않았으나 후속 준비 과정에서 비공개 Alpha, tester/country 설정과 Health·Data safety·광고 ID·카테고리·게시 대기 상태를 직접 확인했다. 아직 저장하거나 제출하지 않은 값은 완료로 간주하지 않았다.
