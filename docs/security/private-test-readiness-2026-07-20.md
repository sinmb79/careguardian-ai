...
- Play Closed Alpha에 동일한 SHA-256의 AAB를 업로드했고 Play가 `6 (1.0.1)`, minSdk 24+, targetSdk 36, ABI 4개로 인식했다. 출시 초안 `1.0.1 (6) - 비공개 테스트`는 저장됐으며 아직 Google 검토로 전송하지 않았다.

Google 검토 제출 전 현재 게이트:
...
5. **미완료:** 현재 선택된 테스터 목록은 합계 기준 최대 9명이다. 의도된 테스터 12명 이상을 확보한 뒤, 보스의 별도 확인을 받아 게시 개요의 13개 변경사항을 Google 검토에 제출한다.
...
- [ ] 게시 개요의 13개 변경사항을 Google 검토에 제출
...
- 최초 감사에서는 Play Console을 열지 않았으나 후속 준비 과정에서 비공개 Alpha, tester/country 설정과 Health·Data safety·광고 ID·카테고리·게시 대기 상태를 직접 확인했다. 저장된 값과 미제출 상태를 구분했으며, 13개 변경사항의 Google 검토 제출은 완료로 간주하지 않았다.
