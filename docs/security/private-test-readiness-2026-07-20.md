...
- Play Closed Alpha에 동일한 SHA-256의 AAB를 업로드했고 Play가 `6 (1.0.1)`, minSdk 24+, targetSdk 36, ABI 4개로 인식했다. 출시 `1.0.1 (6) - 비공개 테스트`와 관련 변경사항을 합친 13개 변경사항을 2026-07-20 Google 검토로 전송했으나, 건강·의료 앱을 개인 개발자 계정으로 제출했다는 `Play Console Requirements` 위반으로 거부됐다. 문제 영역은 `Developer Account`이며 AAB·앱 보안·Data safety 거부가 아니다.
...
6. **차단:** 현재 `22B` 계정은 개인 계정이며 공식 웹사이트도 등록되지 않아 계정 유형 변경이 비활성화되어 있다. 정확한 Health/Medical 선언을 유지하면서 공식 조직 웹사이트와 D-U-N-S 기반 조직 정보를 검증해 조직 계정으로 전환해야 한다.
7. **미완료:** 조직 전환 완료 후 최소 72시간을 기다려 13개 변경사항을 다시 제출하고 Google 승인을 확인한다. 그 다음 실제 고유 opt-in 12명 이상을 14일 연속 유지한다.
...
- [ ] 공식 웹사이트·D-U-N-S로 개인 계정을 조직 계정으로 전환
- [ ] 조직 전환 후 72시간 대기하고 거부된 13개 변경사항 재제출
...
- 최초 감사에서는 Play Console을 열지 않았으나 후속 준비 과정에서 비공개 Alpha, tester/country 설정과 Health·Data safety·광고 ID·카테고리·게시 상태를 직접 확인했다. 2026-07-22에는 정책 상태와 계정 세부정보를 다시 확인해 거부 영역이 `Developer Account`이며 현재 계정이 개인, 공식 웹사이트 미등록, 계정 유형 변경 비활성화 상태임을 확인했다. 조직 전환·재제출·Google 승인과 실제 테스터 opt-in 수는 아직 완료되지 않았다.
