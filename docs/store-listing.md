...
이 문서는 `생활후견 AI 1.1.2 (9)`의 현재 Console 입력 기준입니다. 실제 Console 저장 전에는 새 AAB와 고정 Hugging Face HTTPS 요청 경로 2개를 다시 대조합니다. IARC 등급은 실제 Console 설문 응답으로만 확정하며 이 문서는 임의 등급을 만들지 않습니다.
